...
const DeckTemplate = require.resolve(
  `gatsby-theme-mdx-deck/src/templates/deck`
);
const DecksTemplate = require.resolve(
  `gatsby-theme-mdx-deck/src/templates/decks`
);
...
const resolveFrontmatterField = (fieldName) => async (
  source,
  args,
  context,
  info
...
exports.createPages = async ({ graphql, actions, reporter, pathPrefix }) => {
  ...
  const result = await graphql(`
    {
      allDeck(sort: { order: DESC, fields: order }) {
        edges {
          node {
            id
            slug
